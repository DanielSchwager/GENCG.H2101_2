//https://editor.p5js.org/Arnav-Sirigere/sketches/LXAysJpBj
...
function setup() {
  ...
  let p1 = createP(
    "Enter a number in base 10, where each bit in the binary representation of the number represents the evolution for a corresponding neighborhood"
  );
  ...
  if (validRule) {
    for (let y = 0; y < automata.length; y++) {
      ...
      for (let x = 0; x < generation.length; x++) {
        fill(
          generation[x] == "1" ? colorPicker1.color() : colorPicker2.color()
        );
        noStroke();
    ...
    textSize(20);
    text(
      "Invalid rule! It has to be a number between 0 and 255 inclusively",
      20,
      height / 2
    );
  }
...
function nextGeneration() {
  ...
  for (let i = 0; i < cols; i++) {
    let neighborhood = [
      i > 0 ? prevGen[i - 1] : prevGen[cols - 1],
      prevGen[i],
      i < cols - 1 ? prevGen[i + 1] : prevGen[0],
    ].join("");
...
function computeRules() {
  ...
  for (let i = 0; i < allNeighborhoods.length; i++) {
    evolutions[allNeighborhoods[i]] = rule
      .toString(2)
      .padStart(8, "0")
      .split("")[i];
  }
